...
void extract_audio(){
    ...
    if(re_encode == "y"){
        ...
        string format;
        cin >> format;
        string command = "ffmpeg -i -map 0:a " + file_name + " " + file_name.substr(file_name.find_last_of(".")+ 1) + "_converted." + format;
        exec_print(command.c_str());
    }
    else{
        string command = "ffmpeg -i -map 0:a -c copy " + file_name.substr(0, file_name.find_last_of(".")) + "_audio.wav";
        exec_print(command.c_str());
    }
}
...
void extract_video(){
    ...
    if(re_encode == "y"){
        ...
        string format;
        cin >> format;
        string command = "ffmpeg -i -map 0:v " + file_name + " " + file_name.substr(file_name.find_last_of(".")+ 1) + "_extracted." + format;
        exec_print(command.c_str());
    }
    else{
        string command = "ffmpeg -i -map 0:v -c copy " + file_name.substr(0, file_name.find_last_of(".")) + "_extracted." + file_name.substr(file_name.find_last_of(".")+ 1);
        exec_print(command.c_str());
    }
}
...
void cut_video(){
    ...
    string end_time;
    cin >> end_time;
    string command = "ffmpeg -ss " + start_time + " -i " + file_name + " -to " + end_time + " -c copy " + file_name.substr(0, file_name.find_last_of(".")) + "_cut." + file_name.substr(file_name.find_last_of(".")+ 1);
    exec_print(command.c_str());
}
